...
library(FNN)  # efficient k-nearest neighbors
...
# can be generated with the `scripts/downloadGraphHeaders.py` script. If you add
...
# The file ../data/results_imgseg.csv contains all the results for the
...
raw.results <- read.csv("../data/results.csv", stringsAsFactors = FALSE)
raw.imgseg.results <- read.csv("../data/results_imgseg.csv", stringsAsFactors = FALSE)
...
sum(raw.results$limit[raw.results$graphname %in% int.graphs]) / num.heuristic / 3600 / 20
sum(raw.results$limit) / num.heuristic / 3600 / 20
# Total human dollars to run (per heuristic, $0.07 per hour)
sum(raw.results$limit[raw.results$graphname %in% int.graphs]) / num.heuristic / 3600 * 0.07
sum(raw.results$limit) / num.heuristic / 3600 * 0.07
#-------------------------------------------------------------------------------
...
print("Summary of instance props for standard test bed with known opt:")
...
print(paste("Train size", length(train.graphs), "test size",
...
# Here we calculate difficulties and build trees and plots for all heuristics
# as shown in the paper.
#-------------------------------------------------------------------------------
# Get difficulty for each instance: deviation / median deviation
results.diff <- int.results %>%
  group_by(graphname) %>%
  mutate(normdev = averageDev / median(averageDev)) %>%
...
print(paste("Num with exactly one best", nrow(res.class)))
