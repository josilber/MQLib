...
if len(sys.argv) < 2 or (not (sys.argv[1] == "METRICS" and len(sys.argv) == 3) and not (sys.argv[1] == "FULL" and len(sys.argv) == 5 and sys.argv[3].isdigit() and all([x.isdigit() for x in sys.argv[4].split("_")]))):
    print "Usage:\n  python MQLibMaster.py METRICS tag\n    [[or]]\n  python MQLibMaster.py FULL tag #ITERFORBASELINE SEEDS_SEPARATED_BY_UNDERSCORES"
    exit(1)
...
while True:
    if sys.argv[1] == "METRICS":
        ...
        p = subprocess.Popen(["python", "MQLibRunner.py", sys.argv[1], sys.argv[2], sys.argv[3],
                              sys.argv[4]], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
